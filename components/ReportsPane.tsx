...
import { ArrowDownTrayIcon, ClipboardDocumentListIcon, ExclamationTriangleIcon, TruckIcon } from './icons';
...
type TabType = 'general' | 'missingOA' | 'dispatchPending';
...
const ReportsPane: React.FC<ReportsPaneProps> = ({ purchaseOrders, onUpdatePO }) => {
    ...
    const handleSaveRemark = (po: PurchaseOrder) => {
        ...
        if (newRemark !== undefined) {
            ...
            // clear local edit state to show saved value? Or keep it? keeping it is fine.
    ...
    return (
        <div className="container mx-auto p-4 sm:p-6 lg:p-8 h-full flex flex-col">
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 p-6 flex-1 flex flex-col">
                ...
                {/* Tabs */}
                <div className="flex space-x-1 rounded-xl bg-slate-100 dark:bg-slate-900/50 p-1 mb-6">
                    <button
                    ...
                    >
                        <TruckIcon className="w-4 h-4" /> Ready to Ship (Pending)
                    </button>
                </div>
